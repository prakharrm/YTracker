...
router.get("/tracker/:trackingId", async (req, res) => {
    try {
        ...
        console.log("called backend route 1". processedData)
...
router.get("/summarize-video", async (req, res) => {
    try {
        ...
        try {
            ...
            return res.status(500).json({ error: "Invalid JSON after extraction", rawResponse });
        }

        if (!Array.isArray(summary)) {
            console.error("Summary is not an array:", summary);
            return res.status(500).json({ error: "Summary is not in the expected array format", summary });
        }
